(function ($) {
                ...
                $.getJSON(dataUri + countryCode, function (data) {
                    $.each(addressFields, function (index, fieldName) {
                        var field = fieldset.find('input[name="' + fieldNameFormat.replace(':fieldname:', fieldName) + '"]'),
                            group = field.closest('div.form-group');
    ...
    (function () {
        ...
        function addInputCell(row, input) {
            var td = $('<td/>'),
                formGroup = input.closest('div.form-group');
        ...
        $.fn.sortableItems = function (defaults) {
            ...
            return this.each(function () {
                var container        = $(this),
                    legend           = container.children('legend'),
                    fieldsets        = container.children('fieldset'),
                    template         = $(container.children('span').data('template')),
                    defaultUnit      = null,
                    defaultUnitPrice = null,
                    table = $('<table class="table table-bordered"><thead><tr/></thead><tbody></table>'),
                    ...
                    removeButton = $('<a href="#" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-minus"/></a>'),
                    sortButton   = $('<a href="#" class="btn btn-default btn-sm sort"><span class="glyphicon glyphicon-sort"/></a>'),
                    currentIndex = 0,
                    addButton = $('<a href="#" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-plus"/></a>');
        ...
        $.fn.updateFormFromClient = function (dataUri, options, itemsFieldset) {
            this.on('change', function () {
                var fieldNameFormat = $(this).attr('name').replace('client', ':fieldname:'),
                    clientId        = $(this).val(),
                    fieldset        = $(this).closest('fieldset'),
                    projectsSelect = fieldset.find('select[name="' + fieldNameFormat.replace(':fieldname:', 'project') + '"]');
